...
setwd("~/Desktop/ILCs_21March2023_used")
getwd()
...
setwd("~/Desktop/ILCs_21March2023_used/data/")
...
setwd(PrimaryDirectory)
setwd("~/Desktop/ILCs_21March2023_used/metadata/")
MetaDirectory <- getwd()
setwd(PrimaryDirectory)
...
for(i in transformed.cols){
  make.colour.plot(do.subsample(cell.dat, 20000), i, plot.against, title = cofactor)
}
###set 2
to.asinh <- names(cell.dat)[c(11,15,16)] 
...
###set 3
to.asinh <- names(cell.dat)[c(7,10,12,13)] 
...
###set 4
to.asinh <- names(cell.dat)[c(4,6,8,14)] 
...
cluster.cols <- names(cell.dat)[c(21:26,29:36)] 
as.matrix(cluster.cols)

exp.name <- "77-MRK_ILCs_21March2023"
sample.col <- "Sample"
group.col <- "Group"
batch.col <- "Batch"
...
#### all cell counts of groups in order:  26887, 30779, 29151, 25154
...
make.colour.plot(cell.sub, "tSNE_X", "tSNE_Y", "Population", col.type = 'factor', add.label = FALSE, title = "ILCs")
...
annot.cols <- c(group.col, batch.col)
plotperct.cols <- names(sum.dat)[c(4:15)]
plotperct.cols
...
make.pheatmap(sum.dat, 
             ...
             plot.title = 'Frequency of ILC Pops.,Decidua',
             annot.cols = annot.cols,
              dendrograms = "both",
             file.name = "Frequency of ILC Pops normalized to column.png")
make.pheatmap(sum.dat, 
             sample.col = sample.col, 
             plot.cols = plotperct.cols, 
             plot.title = 'Frequency of ILC cell Pops, Decidua',
             annot.cols = annot.cols,
             dendrograms = "both",
             normalise = FALSE,
             file.name = "Frequency of ILC T cell Pops pops.png")
#make.pheatmap(sum.dat, 
#            sample.col = sample.col, 
#            plot.cols = plotperct.cols, 
